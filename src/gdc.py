...
def depth2ptc(depth, calib):
    ...
    n = points.shape[0]
    pts = np.hstack((points, np.ones((n, 1))))
    pts[:, 0] = pts[:, 0] * pts[:, 2] * 1700 / 256
    pts[:, 1] = pts[:, 1] * pts[:, 2] * 3517 / 512

    # k = [i[0]!= 0 for i in pts]
    # print(pts[k, :])
    # print(depth.shape)
    ...
    return np.dot(pts, np.transpose(calib))[:, :3]
...
def GDC(pred_depth, gt_depth, calib,
    ...
    # i.e. pred_depth is [H, W, 1].
    # pts is [N, 3]
    ptc = depth2ptc(pred_depth, calib)

    # Filter points lie without the region defined.
    # consider_PL is 2-d array with True/False
    ...
    # consider_L correspondint to LiDAR. PL conrrespondint to Pseudo LiDAR.
    consider_L = filter_mask(depth2ptc(gt_depth, calib)
                             ).reshape(gt_depth.shape)

    # retain points in both LiDAR and Pseudo LiDAR
    gt_mask = consider_L * consider_PL

    # We don't drastically move points.
    # This avoids numerical issues in solving linear equations.
    # i.e. if the point depth is extremely different between pred_depth and gt_depth, it will be filtered.
    ...
    return depth2ptc(new_depth_map, calib)